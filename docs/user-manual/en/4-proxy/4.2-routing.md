...
App routing means letting ICodeEasy route a specific application's API requests through the local routing service.
...
When routing is enabled, ICodeEasy modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, ICodeEasy will:
...
Before enabling routing, ICodeEasy backs up the original configuration and restores it when disabled.
