...
This section describes how to configure ICodeEasy according to your preferences.
...
ICodeEasy supports two languages:
...
When enabled, ICodeEasy automatically runs when the system starts.
...
Starting from v3.13.0, ICodeEasy adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger ICodeEasy via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If ICodeEasy runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, ICodeEasy automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that ICodeEasy uses when opening a terminal.
...
The storage location for ICodeEasy's own data, defaulting to `~/.icodeeasy/`.
...
Configure ICodeEasy's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, ICodeEasy automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
- ICodeEasy automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, ICodeEasy displays details about the remote snapshot:
- Protocol version (v2)
...
There is no local callback port. Authentication and model requests use ICodeEasy's global outbound proxy configuration.

The xAI integration does **not** use an OAuth application registered to ICodeEasy. It reuses the public OAuth client identity and scopes registered for the official Grok CLI (`client_id: b1a00492-073a-47ea-816f-4c329264a828`, including `grok-cli:access`) to obtain account-backed API access.
...
Displays the current ICodeEasy version number, with support for:
