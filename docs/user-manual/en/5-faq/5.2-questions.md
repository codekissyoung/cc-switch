...
ICodeEasy for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [official download page](https://icodeeasy.cc/download/).
...
2. Add ICodeEasy to your antivirus software's whitelist
...
chmod +x ICodeEasy-*.AppImage

# If it still fails, try
./ICodeEasy-*.AppImage --no-sandbox
```
...
```bash
CC_SWITCH_GDK_BACKEND=wayland ./ICodeEasy-*.AppImage
```
...
**Solutions**:
1. Check if the `~/.icodeeasy/` directory exists
2. Restore from backup: `~/.icodeeasy/backups/`
...
**Solutions**:
1. Confirm the file was exported by ICodeEasy
...
**Windows**:
- Check taskbar settings to ensure the ICodeEasy icon is not hidden
...
2. Restart the app
3. Delete `~/.icodeeasy/settings.json` to reset settings

### Update Failed

**Solutions**:
1. Check network connection
2. Download the latest version from the [official download page](https://icodeeasy.cc/download/) and install it over the existing copy

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and ICodeEasy runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, ICodeEasy covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `icodeeasy://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit the [ICodeEasy website](https://icodeeasy.cc) and reach out via the contact option on the download page
...
ICodeEasy stores logs under its application configuration directory. By default this is `.icodeeasy` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When reporting a problem, attach the files relevant to the issue:

- General, network, or proxy errors: `~/.icodeeasy/logs/cc-switch.log` and its rotated files
- Application crashes: `~/.icodeeasy/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.icodeeasy\...`
