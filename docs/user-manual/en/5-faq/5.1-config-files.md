...
## ICodeEasy Data Storage

### Storage Directory

Default location: `~/.icodeeasy/`

Customizable location in settings (for cloud sync).

On the first upgrade from an older version, ICodeEasy copies `~/.cc-switch/` to this directory atomically and retains the legacy directory as a rollback copy. If both directories already exist, ICodeEasy uses `~/.icodeeasy/` without merging or overwriting either directory.
...
Hermes uses YAML configuration. ICodeEasy writes MCP servers to `mcp_servers`, writes editable provider entries to `custom_providers`, reads read-only entries from Hermes' `providers` dict, and updates `model.provider` / `model.default` when switching providers.
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to ICodeEasy's "providers") |
...
ICodeEasy's priority when modifying configurations:

1. **ICodeEasy Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by ICodeEasy)
- ICodeEasy's `settings.json`
...
ICodeEasy v3.7.0 migrated from JSON files to SQLite:
