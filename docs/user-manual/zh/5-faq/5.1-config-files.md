...
从旧版本首次升级时，ICodeEasy 会以原子方式将 `~/.cc-switch/` 复制到此目录，并保留旧目录作为回退副本。如果两个目录已经同时存在，ICodeEasy 将直接使用 `~/.icodeeasy/`，不会自动合并或覆盖任一目录。
...
Hermes 使用 YAML 配置。ICodeEasy 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 ICodeEasy 的"供应商"） |
| `env` | 环境变量配置 |
...
- CLI 工具的配置文件（会被 ICodeEasy 回填）
- ICodeEasy 的 `settings.json`
...
ICodeEasy v3.7.0 从 JSON 文件迁移到 SQLite：
