...
Only the latest release of ICodeEasy receives security updates.

仅最新版本的 ICodeEasy 会收到安全更新。
...
ICodeEasy is a local desktop application. It manages configuration files for AI coding CLIs on the user's own machine. There is no project-operated cloud backend, no multi-user model, and no privilege separation from the user who runs it.

ICodeEasy 是一个本地桌面应用，用于管理本机上各 AI 编程 CLI 的配置文件。本项目不运营任何云端后端，没有多用户模型，也不与运行它的用户之间存在权限隔离。
...
**Still in scope:** any complete, demonstrable chain in which an *untrusted* source — a `icodeeasy://` deep link, a remote sync payload, remote data, an inbound proxy request, or an XSS — reaches a high-privilege IPC command. The trust placed in the renderer covers the code we ship, not arbitrary values that flow through it.

**仍在范围内**：任何完整、可演示的利用链，其中**不可信来源**——`icodeeasy://` deeplink、远程同步载荷、远程数据、代理入站请求或 XSS——抵达高权限 IPC 命令。对渲染进程的信任覆盖的是我们发布的代码，而非流经其中的任意值。
...
- `icodeeasy://` deep link payloads / deeplink 载荷（由第三方构造，经浏览器抵达）
...
Instead, please report them through [GitHub Security Advisories](https://github.com/codekissyoung/cc-switch/security/advisories/new).

请通过 [GitHub 安全公告](https://github.com/codekissyoung/cc-switch/security/advisories/new) 进行报告。
...
Security fixes are released as patch versions and announced on the [official download page](https://icodeeasy.cc/download/). We recommend always updating to the latest version.

安全修复通过补丁版本发布，并在[官方下载页](https://icodeeasy.cc/download/)公布。建议始终更新到最新版本。
