//! Panic Hook 模块
//!
//! 在应用崩溃时捕获 panic 信息并记录到 `<app_config_dir>/crash.log` 文件中（默认 `~/.icodeeasy/crash.log`）。
//! 便于用户和开发者诊断闪退问题。
...
mod tests {
    ...
    fn test_crash_log_path() {
        ...
        assert!(matches!(
            directory_name,
            Some(crate::config::APP_CONFIG_DIR_NAME)
                | Some(crate::config::LEGACY_APP_CONFIG_DIR_NAME)
        ));
    }
